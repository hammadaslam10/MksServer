...
exports.CreateRace = Trackerror(async (req, res, next) => {
  ...
  schedule.scheduleJob(`'${data._id}'`, Timing, function () {
    LoadStatus();
    console.log("done");
  });

  console.log(schedule.scheduledJobs[`'${data._id}'`]);
...
exports.EditRace = Trackerror(async (req, res, next) => {
  ...
  if (req.files == null) {
    ...
    if (!schedule.scheduledJobs[`'${req.params.id}'`]) {
      schedule.scheduleJob(`'${req.params.id}'`, Timing, function () {
        LoadStatus();
        console.log("udpatedracecronjob");
      });
    } else {
